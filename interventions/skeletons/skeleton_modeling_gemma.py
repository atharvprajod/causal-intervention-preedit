...
class SkeletonGPT2Layer(nn.Module):
    def __init__(self, config, intermediate_size):
        super().__init__()
        # Initialize the attention layer
        self.attention = GPT2Attention(config)
        # Initialize the position-wise feed-forward network
        self.mlp = GPT2MLP(intermediate_size, config) # Pass the intermediate_size here
        # Initialize the layer normalization
        self.ln_f = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_epsilon)

    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        # Apply the attention layer
        attn_output = self.attention(hidden_states, attention_mask, head_mask)
        # Apply the position-wise feed-forward network
        hidden_states = self.mlp(attn_output)
        # Apply layer normalization
        hidden_states = self.ln_f(hidden_states)
        return hidden_states


class SkeletonGPT2Model(nn.Module):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([SkeletonGPT2Layer(config) for _ in range(config.num_hidden_layers)])
...
# model = SkeletonGPT2Model(config)
